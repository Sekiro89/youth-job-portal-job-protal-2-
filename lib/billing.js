...
async function stripe() {
  ...
  if (_client.key !== key) _client = { key, client: require('stripe')(key, { appInfo: { name: 'Youth Futures Canada', url: await publicUrl() } }) };
...
async function billFrom() {
  ...
  return { site_name: s.site_name || 'Youth Futures Canada', contact_address: (s.contact_address || '').trim(), public_url: (s.public_url || 'http://localhost:3900').replace(/\/$/, ''), gst_number: (s.gst_number || '').trim() || null };
}
...
async function ensureWebhook(client, url, { log = console.log, create = true } = {}) {
  ...
  const ep = await client.webhookEndpoints.create({ url, enabled_events: WEBHOOK_EVENTS, description: 'Youth Futures Canada — job posting subscriptions' });
