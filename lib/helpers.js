...
const formatPostal = (s) => String(s || '').toUpperCase().replace(/\s+/g, '').replace(/^(.{3})(.{3})$/, '$1 $2');
...
module.exports = {
  ...
  location, fullAddress, displayCompany, legalNameNote, formatPostal, statusLabel, statusBadge,
  ...
  formatHours: (job) => (job && job.hours_amount ? `${Number(job.hours_amount) % 1 === 0 ? Number(job.hours_amount) : job.hours_amount} hours ${C.HOURS_PERIOD_NAME[job.hours_period] || 'per week'}` : ''),
