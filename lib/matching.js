  ...
  for (const s of seekers) {
    ...
    if (s.notify_email && s.notify_frequency === 'instant') {
      ...
      await mail.send({
        ...
        html: mail.layout(`A new job matches your profile`, `<p>Hi ${h.escapeHtml(s.name || 'there')},</p><p>A new posting on Youth Careers Canada matches the categories, keywords or skills in your profile:</p>${jobEmailBlock(job)}<p>You can apply in one click with your saved resume.</p><p style="font-size:13px;color:#6b7a8c">Manage your alerts at ${h.escapeHtml(PUBLIC_URL)}/jobseeker/alerts</p>`, { href, label: 'View this job' }),
