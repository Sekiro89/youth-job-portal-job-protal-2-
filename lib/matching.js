...
async function countMatchesForSeeker(userId) {
  const r = await db.one(`SELECT count(*)::int AS n FROM seeker_profiles p, jobs
    WHERE p.user_id = $1 AND ${PUBLIC_WHERE} AND ${PROVINCE_OK} AND (${CATEGORY_MATCH} OR ${KEYWORD_MATCH} OR ${SKILL_MATCH})`, [userId]);
  return r ? r.n : 0;
}

/** Public jobs scored against the seeker's profile (3 category + 2 keyword + 1 skill + 1 province + 1 audience), best first. */
async function matchesForSeeker(userId, limit = 6) {
  return db.many(`
    SELECT jobs.*, ep.company_name, ep.slug AS company_slug, ${SCORE} AS score, ${AUDIENCE_HIT} AS audience_hit,
