...
// province:  seeker has no province preference, or job province is one of them, or the job is remote
const CATEGORY_MATCH = `jobs.category = ANY(p.categories)`;
const KEYWORD_MATCH = `EXISTS (SELECT 1 FROM unnest(p.keywords) k
  WHERE length(trim(k)) > 0 AND (jobs.title ILIKE '%' || trim(k) || '%' OR jobs.description ILIKE '%' || trim(k) || '%'
        OR EXISTS (SELECT 1 FROM unnest(jobs.skills) js WHERE js ILIKE '%' || trim(k) || '%')))`;
const SKILL_MATCH = `EXISTS (SELECT 1 FROM unnest(jobs.skills) js JOIN unnest(p.skills) ps ON lower(trim(js)) = lower(trim(ps)))`;
const PROVINCE_OK = `(cardinality(p.provinces) = 0 OR jobs.province = ANY(p.provinces) OR jobs.work_arrangement = 'remote')`;
const PROVINCE_HIT = `(jobs.province = ANY(p.provinces) OR jobs.work_arrangement = 'remote')`;

const SCORE = `((CASE WHEN ${CATEGORY_MATCH} THEN 3 ELSE 0 END) + (CASE WHEN ${KEYWORD_MATCH} THEN 2 ELSE 0 END)
  + (CASE WHEN ${SKILL_MATCH} THEN 1 ELSE 0 END) + (CASE WHEN ${PROVINCE_HIT} THEN 1 ELSE 0 END))`;

function jobEmailBlock(job) {
  return `<p><strong>${h.escapeHtml(job.title)}</strong><br>${h.escapeHtml(job.company_name)} · ${h.escapeHtml(h.location(job))}<br>
  <span style="color:#5A6B7E">${h.escapeHtml(h.jobTypeName(job.job_type))} · ${h.escapeHtml(h.workArrangementName(job.work_arrangement))} · ${h.escapeHtml(h.formatSalary(job))}</span></p>`;
}
...
async function notifySeekersForJob(jobId) {
  const job = await db.one(`SELECT jobs.*, ep.company_name FROM jobs JOIN employer_profiles ep ON ep.id = jobs.employer_profile_id WHERE jobs.id = $1`, [jobId]);
  if (!job) return 0;
  ...
  for (const s of seekers) {
    const body = `${job.company_name} · ${h.location(job)} · ${h.formatSalary(job)}`;
    ...
    if (s.notify_email && s.notify_frequency === 'instant') {
      const href = mail.PUBLIC_URL + link;
      await mail.send({
        to: s.email,
        subject: `New job match: ${job.title} at ${job.company_name}`,
        html: mail.layout(`A new job matches your profile`, `<p>Hi ${h.escapeHtml(s.name || 'there')},</p><p>A new posting on Canada Careers matches the categories, keywords or skills in your profile:</p>${jobEmailBlock(job)}<p>You can apply in one click with your saved resume.</p><p style="font-size:13px;color:#6b7a8c">Manage your alerts at ${h.escapeHtml(mail.PUBLIC_URL)}/jobseeker/alerts</p>`, { href, label: 'View this job' }),
        text: `Hi ${s.name || 'there'},\n\nA new job matches your profile: ${job.title} at ${job.company_name} (${h.location(job)}).\n${href}\n\nManage your alerts: ${mail.PUBLIC_URL}/jobseeker/alerts`,
      });
...
async function sendDailyDigests() {
  ...
  let sent = 0;
  for (const r of rows) {
    const list = r.items.map(i => `<li style="margin:0 0 10px"><a href="${h.escapeHtml(mail.PUBLIC_URL + i.link)}" style="font-weight:600;color:#1F3A5F">${h.escapeHtml(i.title.replace(/^New job: /, ''))}</a><br><span style="color:#5A6B7E;font-size:14px">${h.escapeHtml(i.body || '')}</span></li>`).join('');
    const text = r.items.map(i => `- ${i.title.replace(/^New job: /, '')} — ${i.body || ''}\n  ${mail.PUBLIC_URL + i.link}`).join('\n');
    await mail.send({
      to: r.email,
      subject: `${r.items.length} new job${r.items.length === 1 ? '' : 's'} matched your profile today`,
      html: mail.layout('Your daily job matches', `<p>Hi ${h.escapeHtml(r.name || 'there')},</p><p>These postings from the last 24 hours match your Canada Careers profile:</p><ul style="padding-left:18px">${list}</ul><p style="font-size:13px;color:#6b7a8c">Manage your alerts at ${h.escapeHtml(mail.PUBLIC_URL)}/jobseeker/alerts</p>`, { href: `${mail.PUBLIC_URL}/jobseeker/dashboard`, label: 'See all matches' }),
...
/** Public jobs scored against the seeker's profile (3 category + 2 keyword + 1 skill + 1 province), best first. */
async function matchesForSeeker(userId, limit = 6) {
  return db.many(`
    SELECT jobs.*, ep.company_name, ep.slug AS company_slug, ${SCORE} AS score,
    ...
    WHERE p.user_id = $1 AND ${PUBLIC_WHERE} AND ${PROVINCE_OK}
      AND (${CATEGORY_MATCH} OR ${KEYWORD_MATCH} OR ${SKILL_MATCH})
    ORDER BY score DESC, jobs.published_at DESC NULLS LAST
    LIMIT $2`, [userId, limit]);
}

module.exports = { notifySeekersForJob, sendDailyDigests, matchesForSeeker };
