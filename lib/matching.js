...
async function notifySeekersForJob(jobId) {
  const job = await db.one(`SELECT jobs.*, ep.company_name, ep.operating_name, ${LOCATION_COUNT} FROM jobs JOIN employer_profiles ep ON ep.id = jobs.employer_profile_id WHERE jobs.id = $1`, [jobId]);
  if (!job) return 0;
  ...
  for (const s of seekers) {
    const body = `${company} · ${h.location(job)} · ${h.formatSalary(job)}`;
...
async function matchesForSeeker(userId, limit = 6) {
  return db.many(`
    SELECT jobs.*, ep.company_name, ep.operating_name, ep.slug AS company_slug, ${LOCATION_COUNT}, ${SCORE} AS score, ${AUDIENCE_HIT} AS audience_hit,
