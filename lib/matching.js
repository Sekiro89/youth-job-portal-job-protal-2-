...
function jobEmailBlock(job) {
  ...
  <span style="color:#5A6B7E">${h.escapeHtml(h.jobTypeName(job.job_type))} · ${h.escapeHtml(h.workArrangementName(job.work_arrangement))}${hours ? ` · ${h.escapeHtml(hours)}` : ''} · ${h.escapeHtml(h.formatSalary(job))}</span></p>`;
}
...
async function notifySeekersForJob(jobId) {
  const job = await db.one(`SELECT jobs.*, ${JOB_COMPANY}, ${LOCATION_COUNT} FROM jobs JOIN employer_profiles ep ON ep.id = jobs.employer_profile_id WHERE jobs.id = $1`, [jobId]);
  if (!job) return 0;
  ...
  for (const s of seekers) {
    const body = `${company} · ${h.location(job)}${hours ? ` · ${hours}` : ''} · ${h.formatSalary(job)}`;
    ...
    if (s.notify_email && s.notify_frequency === 'instant') {
      ...
      await mail.send({
        ...
        text: `Hi ${s.name || 'there'},\n\nA new job matches your profile: ${job.title} at ${company} (${h.location(job)}${hours ? `, ${hours}` : ''}).\n${href}\n\nManage your alerts: ${PUBLIC_URL}/jobseeker/alerts`,
      });
...
async function sendDailyDigests() {
  ...
  const PUBLIC_URL = await publicUrl();
  for (const r of rows) {
    const list = r.items.map(i => `<li style="margin:0 0 10px"><a href="${h.escapeHtml(PUBLIC_URL + i.link)}" style="font-weight:600;color:#1F3A5F">${h.escapeHtml(i.title.replace(/^New job: /, ''))}</a><br><span style="color:#5A6B7E;font-size:14px">${h.escapeHtml(i.body || '')}</span></li>`).join('');
