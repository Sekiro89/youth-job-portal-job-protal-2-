...
function layout(title, bodyHtml, cta, opts) {
  ...
  const site = (opts && opts.siteName) || 'Youth Futures Canada';
  const button = cta ? `<p style="margin:24px 0"><a href="${escapeHtml(cta.href)}" style="background:#0F2444;color:#fff;text-decoration:none;padding:12px 20px;border-radius:8px;font-weight:600;display:inline-block">${escapeHtml(cta.label)}</a></p>` : '';
...
<div style="max-width:560px;margin:0 auto;padding:24px">
  <div style="background:#fff;border-radius:12px;padding:28px;border:1px solid #e1e7ef">
    ...
    <h1 style="font-size:20px;margin:0 0 12px;color:#0F2444">${escapeHtml(title)}</h1>
    <div style="font-size:15px;line-height:1.55">${bodyHtml}</div>
    ${button}
  </div>
  <p style="font-size:12px;color:#6b7a8c;text-align:center;margin-top:16px">${escapeHtml(site)} · Our dreams. Our skills. Our future. Our Canada.<br>
