...
async function welcome(user, role) {
  ...
  const bodies = {
    ...
    seeker: [`<p>Hi ${first}, welcome to Youth Careers Canada.</p><p>Upload your resume, set your job preferences and we will email you when new postings match. Applying takes one click.</p>`, { href: `${PUBLIC_URL}/jobseeker/profile`, label: 'Complete your profile' }],
  };
  const [html, cta] = bodies[role];
  await mail.send({
    to: user.email, subject: 'Welcome to Youth Careers Canada',
    html: mail.layout('Welcome to Youth Careers Canada', html, cta),
    text: `Hi ${user.name}, welcome to Youth Careers Canada. Get started: ${cta.href}`,
  });
}
...
router.get('/signup', (req, res) => {
  ...
  rememberNext(req);
  res.render('auth/signup', page({ title: 'Create your account', metaDescription: 'Join Youth Careers Canada as an employer ($14.99 per posting per month + GST), third party consultant ($9.99 + GST) or job seeker — applying is always free.' }));
});

const signupPage = { employer: 'auth/signup-employer', consultant: 'auth/signup-consultant', seeker: 'auth/signup-seeker' };
const signupMeta = {
  employer: { title: 'Sign up as an employer', metaDescription: 'Create a Youth Careers Canada employer account and post jobs for your company for $14.99 per posting per month + GST.' },
  consultant: { title: 'Sign up as a third party consultant', metaDescription: 'Create a Youth Careers Canada consultant account to manage job postings for many employers under one login.' },
  seeker: { title: 'Sign up as a job seeker', metaDescription: 'Create a free Youth Careers Canada job seeker account. Upload your resume, apply online and get job alerts by email.' },
};
...
// ---------------------------------------------------------------- login / logout
const loginMeta = { title: 'Sign in', metaDescription: 'Sign in to your Youth Careers Canada account to post jobs, manage employer profiles, or apply to jobs and manage your alerts.' };
...
// ---------------------------------------------------------------- forgot / reset
const forgotMeta = { title: 'Reset your password', metaDescription: 'Request a password reset link for your Youth Careers Canada account.', noindex: true };
...
router.post('/forgot', wrap(async (req, res) => {
  ...
  if (user) {
    ...
    await mail.send({
      to: user.email, subject: 'Reset your Youth Careers Canada password',
      ...
      text: `Reset your Youth Careers Canada password (valid 1 hour): ${link}`,
    });
...
const resetMeta = { title: 'Choose a new password', metaDescription: 'Set a new password for your Youth Careers Canada account.', noindex: true };
...
// ---------------------------------------------------------------- account
const accountMeta = { title: 'My account', metaDescription: 'Manage your Youth Careers Canada account details and password.', noindex: true };
