...
Last verified: 2026-09-09 on the isolated instance (port 3906): ~1,490 words of prose, 2,300 words including Quick answers + FAQ; 5 JSON-LD blocks parse; FAQ JSON-LD mirrors the visible FAQ text verbatim; 41 unique internal links. Pricing copy re-verified 2026-09-09 (pricing round, port 3905) after the two-rate change: FAQ JSON-LD still mirrors the page, both Offers parse.
...
| Meta description (157 chars) | `Canada Careers is Canada’s inclusive job bank: employers post jobs for $14.99 + GST a month, consultants for $9.99 + GST; job seekers apply free with matched alerts.` |
...
| Facts strip (GEO) | $14.99 + GST per employer posting per month ($15.74) · $9.99 + GST per consultant posting ($10.49) | 5 audiences; 13 provinces & territories; 20 job categories; free for job seekers; monthly renewal |
| `quick-answers` (AEO) | What is Canada Careers | who can post jobs; how much does a job posting cost (employer $14.99 / consultant $9.99 + GST); what a posting includes (full work address, several addresses, operating name, salary period, NOC); free for job seekers; how job alerts work; who we serve |
...
| `employers` | post a job in Canada · $14.99 job posting | job board for employers in Canada; job posting pricing; GST; price locked in per posting; receipts downloadable as PDF; full work address / several addresses; operating name; salary period; NOC code; monthly renewal; cancel any time; archived / expired / cancelled / inactive postings |
| `consultants` | third party recruiter job posting · $9.99 consultant rate | staffing agencies; recruitment firms; immigration consultants; HR consultants; many employer profiles; client operating name; single sign-in |
...
| `faq` | job posting cost (two rates) / GST / full work address + operating name / cancel / archive / consultants / resume formats / alerts / coverage / accessibility / PIPEDA / contact | 13 questions, natural-language question headings |
...
| # | `@type` | `@id` | Notes |
|---|---|---|---|
| 1 | `Organization` | `${PUBLIC_URL}/#organization` | name, alternateName, url, `logo` = `/img/logo-stacked.svg`, slogan, description, `sameAs: []` (fill when social profiles exist), `areaServed` Country=Canada, `knowsAbout` (14 topics), `contactPoint` → `/contact`, `makesOffer` → **two** Offers — employer $14.99 CAD / month (url `/employer`) and third party consultant $9.99 CAD / month (url `/consultant`) — each with `UnitPriceSpecification` (`valueAddedTaxIncluded: false`, `unitCode: MON`) |
...
| 5 | `FAQPage` | `${PUBLIC_URL}/about#faq` | 13 Question/Answer pairs; text is the same string rendered in the `<details>` accordions (single source: `FAQ` array in `routes/about.js`; prices come from `lib/constants` `priceCentsFor(role)`) |
...
7. **Employer / pricing page schema** — add `Product` + `Offer` (`$14.99 CAD` on `/employer`, `$9.99 CAD` on `/consultant`, `priceValidUntil`, `UnitPriceSpecification`) and an `HowTo` block ("How to post a job on Canada Careers") on `/employer` so pricing snippets appear for "job posting cost Canada" queries.
