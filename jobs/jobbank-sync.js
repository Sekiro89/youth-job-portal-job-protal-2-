...
async function syncJobBank({ log = console.log, limit = 100, perQuery = 1, maxRequests = 700 } = {}) {
  ...
  // Kill switch: the `jobbank_sync` runtime setting (lib/settings: DB > env JOBBANK_SYNC > 'on'). `scripts/import-jobbank.js --purge`
  // archives every imported posting and sets it 'off'; the admin panel (/admin/integrations) toggles the same key.
  // Nothing is fetched or revived until it is 'on' again (`--enable` or the panel).
