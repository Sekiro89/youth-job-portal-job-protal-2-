...
- 2026-09-15 — client: imported postings must not reveal their origin (Service Canada must not think the site is populated from Job Bank). Removed every public "Job Bank / reference posting / source" badge, attribution card, facts row and the 228 auto-generated "Employer listed on Job Bank" company blurbs; the apply button is the generic "Apply on other platform" and now goes through `GET /jobs/:slug/go` (302 to the stored external URL, noindex, robots-disallowed) so the destination never appears in page source; JSON-LD `url` stays on our domain. Landing copy "Job Bank-style postings" → "Complete, transparent postings". The source is still visible in `/admin/jobs` and in `jobs.source/source_url` for the daily refresh.
- 2026-09-15 — **Domain moved to https://canadacareers.jobs** (A record by Ankit). Caddy: apex serves the app; `www.canadacareers.jobs` and the old `jobs.khosha.tech` 301 to the apex. `settings.public_url` (admin panel) = https://canadacareers.jobs, so canonical/sitemap/robots/emails use it; `.env PUBLIC_URL` is only the fallback. Stripe webhook URL for go-live is now `https://canadacareers.jobs/billing/webhook`; Google Maps key referrer restriction must allow `canadacareers.jobs`. Git history rewritten to drop `docs/.dbpw` before the first push to GitHub.
