...
app.use(async (req, res, next) => {
  try { res.locals.site = await settings.getMany(['site_name', 'contact_phone', 'contact_address', 'support_name', 'public_url']); } catch (e) { res.locals.site = { site_name: 'Youth Careers Canada' }; }
  res.locals.h = h;
  res.locals.C = C;
  ...
  res.locals.title = 'Youth Careers Canada';
  res.locals.metaDescription = 'Youth Careers Canada is a Canadian job bank connecting employers and third-party consultants with young talent everywhere — students, graduates, young professionals and skilled workers. Post a job from $9.99/month + GST (consultants) or $14.99/month + GST (employers).';
...
if (require.main === module) {
  app.listen(PORT, '127.0.0.1', () => console.log(`Youth Careers Canada listening on http://127.0.0.1:${PORT} (${process.env.NODE_ENV || 'development'})`));
}
module.exports = app;
